...
from hyperliquid_trading_agent.app.engine.diagnostics import build_strategy_funnel
...
_DASHBOARD_FUNNEL_LIMIT = 1_000
_DASHBOARD_SIGNAL_QUALITY_LIMIT = 5_000
_DASHBOARD_OPTIONAL_TIMEOUT_SECONDS = 12.0
_DASHBOARD_REGIME_POINTS_PER_ASSET = 60
_DASHBOARD_STRATEGY_GROUP_LIMIT = 200
_DASHBOARD_SIGNAL_GROUP_LIMIT = 150
...
def register_dashboard_routes(app: FastAPI, settings: Settings, require_auth: RequireAuth) -> None:
    ...
    async def unified_dashboard_data(authorization: str | None = Header(default=None)) -> dict[str, Any]:
        ...
        readiness_limit = max(
            1_000,
            int(settings.engine_readiness_min_candidates),
            int(settings.engine_readiness_min_shadow_intents),
        )
        validation = await build_engine_validation_report(
            repository,
            limit=readiness_limit,
            settings=settings,
        )
        readiness, latest_replay, proposals, risk_decisions, regime, diagnostics, signal_quality, feedback_data, counterfactual = await asyncio.gather(
            ...
            latest_engine_replay_comparison(repository),
            _dashboard_rows(repository, "list_candidate_config_diffs", limit=20),
            _dashboard_rows(repository, "list_risk_gateway_decisions", limit=20),
            _regime_dashboard_data(
                repository,
                settings,
                per_asset_limit=_DASHBOARD_REGIME_POINTS_PER_ASSET,
            ),
            _safe_dashboard_component(_dashboard_diagnostics(repository)),
            _safe_dashboard_component(
                build_signal_quality_report(
                    repository,
                    window_hours=24,
                    max_rows=_DASHBOARD_SIGNAL_QUALITY_LIMIT,
                )
            ),
            _safe_dashboard_component(
                _dashboard_feedback_data(repository)
            ),
            _safe_dashboard_component(latest_news_risk_counterfactual(repository)),
        )
        candidate_funnel = diagnostics.get("candidate_funnel") or diagnostics
        strategy_funnel = _bounded_dashboard_groups(
            diagnostics.get("strategy_funnel") or diagnostics,
            limit=_DASHBOARD_STRATEGY_GROUP_LIMIT,
        )
        signal_quality = _bounded_dashboard_groups(
            signal_quality,
            limit=_DASHBOARD_SIGNAL_GROUP_LIMIT,
        )
        readiness = _dashboard_readiness_summary(readiness)
        feedback = feedback_data.get("feedback") or feedback_data
        publisher_start_ms = int(feedback_data.get("publisher_cohort_start_ms") or 0)
...
async def _safe_dashboard_component(
    awaitable: Any,
    *,
    timeout_seconds: float = _DASHBOARD_OPTIONAL_TIMEOUT_SECONDS,
) -> Any:
    try:
        return await asyncio.wait_for(awaitable, timeout=max(0.1, timeout_seconds))
    except TimeoutError:
...
async def _dashboard_rows(repository: Any, method_name: str, *, limit: int) -> list[dict[str, Any]]:
...
async def _dashboard_diagnostics(repository: Any) -> dict[str, Any]:
    strategy_funnel = await build_strategy_funnel(
        repository,
        window_hours=24,
        limit=_DASHBOARD_FUNNEL_LIMIT,
    )
    return {
        "candidate_funnel": strategy_funnel.get("candidate_funnel") or {},
        "strategy_funnel": {
            key: value
            for key, value in strategy_funnel.items()
            if key != "candidate_funnel"
        },
    }


async def _dashboard_feedback_data(repository: Any) -> dict[str, Any]:
    publisher_start_ms = await _publisher_start_ms(repository)
    feedback = await build_newswire_feedback_summary(
        repository,
        cohort_start_ms=publisher_start_ms,
    )
    return {
        "feedback": feedback,
        "publisher_cohort_start_ms": publisher_start_ms,
    }


def _dashboard_readiness_summary(readiness: dict[str, Any]) -> dict[str, Any]:
    keys = (
        "generated_at_ms",
        "window",
        "ready_for_paper",
    ...
    return {
        **{key: readiness.get(key) for key in keys},
        "detail_endpoint": "/engine/readiness",
    }


def _bounded_dashboard_groups(payload: Any, *, limit: int) -> dict[str, Any]:
    if not isinstance(payload, dict):
        return {"status": "unavailable", "error": "InvalidPayload"}
    groups = payload.get("groups")
    if not isinstance(groups, list):
        return payload
    bounded = sorted(
        groups,
        key=lambda item: -int((item or {}).get("evaluation_count") or (item or {}).get("n") or 0),
    )[: max(1, limit)]
    return {
        **payload,
        "groups": bounded,
        "group_count_total": len(groups),
        "groups_truncated": len(groups) > len(bounded),
    }
