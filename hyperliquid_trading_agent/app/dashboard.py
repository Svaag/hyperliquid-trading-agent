...
<section id="readiness" class="tab active panel"></section><section id="regime" class="tab panel"></section><section id="engine" class="tab panel"></section><section id="replay" class="tab panel"></section><section id="pnl" class="tab panel"></section><section id="governance" class="tab panel"></section><section id="raw" class="tab panel"><pre id="rawpre"></pre></section></main>
<script>
...
async function load(){try{$('token').value=localStorage.getItem('agentToken')||'';const d=await api('/dashboard/data');lastData=d;const r=d.engine.readiness||{}, v=d.engine.validation_report||{}, s=v.summary||{}, rep=d.engine.latest_replay_comparison||{}, reg=d.engine.regime||{}, latest=latestRows(reg), tiers=latest.map(x=>x.asset+':'+x.news_risk_tier).join(' ')||'n/a';$('summary').innerHTML=[metric('Paper readiness',r.ready_for_paper?'READY':'BLOCKED',r.ready_for_paper?'ok':'bad'),metric('Score',r.score??0,(r.score>=85?'ok':r.score>=70?'warn':'bad')),metric('Regime',tiers),metric('Regime changes',(reg.changes||[]).length),metric('Shadow intents',s.shadow_intent_count??0),metric('Risk rejects',s.risk_reject_count??0)].join('');$('readiness').innerHTML=`<h2>Readiness: ${esc(r.grade)}</h2><p>Recommendation: <span class="pill">${esc(r.recommendation)}</span></p><h3>Hard blocks</h3>${list(r.hard_blocks)}<h3>Warnings</h3>${list(r.warnings)}<h3>Metrics</h3><pre>${esc(JSON.stringify(r.metrics,null,2))}</pre>`;renderRegime(reg);$('engine').innerHTML=`<h2>Engine validation</h2><pre>${esc(JSON.stringify({status:d.engine.status,validation:v,monitor:d.engine.validation_monitor},null,2))}</pre>`;$('replay').innerHTML=`<h2>Shadow replay</h2><button onclick="runReplay()">Run 24h default comparison</button><pre>${esc(JSON.stringify(rep,null,2))}</pre>`;$('pnl').innerHTML=`<h2>PnL attribution</h2><pre>${esc(JSON.stringify({loop:d.engine.pnl_attribution,pnl:v.pnl_attribution_by_strategy,ev:v.ev_calibration},null,2))}</pre>`;$('governance').innerHTML='<h2>Governance</h2>'+table(d.governance.proposals,[['ID','proposal_id'],['Status','status'],['Strategy','strategy_id']])+'<h3>Risk decisions</h3>'+table(d.governance.risk_decisions,[['ID','decision_id'],['Decision','decision'],['Intent','intent_id']]);$('rawpre').textContent=JSON.stringify(d,null,2)}catch(e){$('summary').innerHTML=`<div class="panel bad"><b>Load failed</b><pre>${esc(e.message)}</pre></div>`}}load();
</script></body></html>
""".strip()
