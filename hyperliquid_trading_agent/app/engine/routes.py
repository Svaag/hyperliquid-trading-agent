...
from hyperliquid_trading_agent.app.engine.strategy_research import build_strategy_research_report
...
async def _enqueue_command(
    repo: Any, *, target_role: str, command_type: str, payload: dict[str, Any]
) -> dict[str, Any]:
    if callable(getattr(repo, "enqueue_worker_command", None)):
        return await repo.enqueue_worker_command(
            target_role=target_role, command_type=command_type, payload=payload, requested_by="api"
        )
    return {
        "command_id": f"unpersisted_{command_type}",
...
def _strategy_catalog_from_specs(specs: list[dict[str, Any]], *, mode: str) -> dict[str, Any]:
        ...
        families.append(
            {
                ...
                "paper_eligible": len(
                    [spec for spec in family_specs if spec.get("enabled") and _spec_paper_eligible(spec)]
                ),
    ...
    return {
        ...
        "spec_only_ids": sorted(
            str(spec.get("strategy_id")) for spec in specs if str(spec.get("strategy_id")) not in runtime_ids
        ),
        "families": families,
    }
...
def _spec_paper_eligible(spec: dict[str, Any]) -> bool:
    ...
    return (
        bool(metadata.get("paper_eligible", True))
        and str(metadata.get("activation_scope") or "paper_shadow") != "shadow_only"
    )


def _spec_shadow_only(spec: dict[str, Any]) -> bool:
    metadata = _spec_metadata(spec)
    return str(metadata.get("activation_scope") or "paper_shadow") == "shadow_only" or bool(
        metadata.get("operator_promotion_required")
    )
...
def register_engine_routes(app: FastAPI, settings: Settings, require_auth: RequireAuth) -> None:
    ...
    async def engine_status(authorization: str | None = Header(default=None)) -> dict[str, Any]:
        ...
        news_status = (
            news_consumer.status()
            if news_consumer is not None and callable(getattr(news_consumer, "status", None))
            else {}
        )
        ...
        newsfeed_health = build_engine_newsfeed_health(
            ...
            latest_source_at_ms=(
                int(latest_newswire_stories[0].get("last_updated_at_ms") or 0) if latest_newswire_stories else None
            ),
        )
        ...
        return {
            ...
            "wave_policy": {
                ...
                if settings.engine_alpha_catalog_mode == "integrated"
                else "disabled",
            },
            ...
            or {
                **monitor_status,
                "running": False,
                "owner_role": "trader",
                "runtime_source": "awaiting_trader_heartbeat",
            },
            "pnl_attribution": pnl_status,
            "debate": {
                "enabled": settings.engine_debate_enabled,
                "max_per_day": settings.engine_debate_max_per_day,
                "priority_min": settings.engine_debate_priority_min,
            },
            "retention": {
    ...
    async def engine_regime_latest(
        primary_asset: str | None = None, authorization: str | None = Header(default=None)
    ) -> dict[str, Any]:
        _auth(authorization)
    ...
    async def engine_regime_history(
    ...
    async def engine_ev_estimates(
        candidate_id: str | None = None, limit: int = 100, authorization: str | None = Header(default=None)
    ) -> list[dict[str, Any]]:
        _auth(authorization)
        return await _repo().list_ev_estimates(candidate_id=candidate_id, limit=limit)

    @app.get("/engine/allocations")
    async def engine_allocations(
        candidate_id: str | None = None, limit: int = 100, authorization: str | None = Header(default=None)
    ) -> list[dict[str, Any]]:
        _auth(authorization)
    ...
    async def engine_strategies(
        family: str | None = None, enabled: bool | None = None, authorization: str | None = Header(default=None)
    ) -> list[dict[str, Any]]:
        _auth(authorization)
        service = getattr(app.state, "engine_service", None)
    ...
    async def engine_strategy_regime_performance(
        strategy_id: str | None = None,
        ...
        _auth(authorization)
        return await _repo().list_strategy_regime_performance(
            strategy_id=strategy_id, regime_label=regime_label, limit=limit
        )

    @app.get("/engine/strategy-regime-performance/{strategy_id}")
    async def engine_strategy_regime_performance_for_strategy(
        strategy_id: str, limit: int = 100, authorization: str | None = Header(default=None)
    ) -> list[dict[str, Any]]:
        _auth(authorization)
    ...
    async def engine_strategy_regime_performance_refresh(
        request: EngineStrategyRegimeRefreshRequest, authorization: str | None = Header(default=None)
    ) -> dict[str, Any]:
        _auth(authorization)
        command = await _enqueue_command(
            _repo(),
            target_role="trader",
            command_type="engine_strategy_regime_refresh",
            payload=request.model_dump(mode="json"),
        )
        return _accepted_command(command)

    @app.post("/engine/position-theses/cleanup")
    async def engine_position_thesis_cleanup(
        request: EnginePositionThesisCleanupRequest, authorization: str | None = Header(default=None)
    ) -> dict[str, Any]:
        _auth(authorization)
        command = await _enqueue_command(
            _repo(),
            target_role="trader",
            command_type="engine_position_thesis_cleanup",
            payload=request.model_dump(mode="json"),
        )
        return _accepted_command(command)

    @app.get("/engine/candidate-trade-packets")
    async def engine_candidate_trade_packets(
        ...
        _auth(authorization)
        return await _repo().list_candidate_trade_packets(
            candidate_id=candidate_id, strategy_id=strategy_id, limit=limit
        )

    @app.get("/engine/candidate-evidence-links")
    async def engine_candidate_evidence_links(
        candidate_id: str | None = None,
        strategy_id: str | None = None,
        limit: int = 100,
        authorization: str | None = Header(default=None),
        ...
        _auth(authorization)
        return await _repo().list_candidate_evidence_links(
            candidate_id=candidate_id, strategy_id=strategy_id, limit=limit
        )

    @app.get("/engine/candidate-outcome-attributions")
    async def engine_candidate_outcome_attributions(
        candidate_id: str | None = None,
        strategy_id: str | None = None,
        ...
        _auth(authorization)
        return await _repo().list_candidate_outcome_attributions(
    ...
    async def engine_council_reviews(
        candidate_id: str | None = None,
        strategy_id: str | None = None,
        ...
        _auth(authorization)
        return await _repo().list_council_reviews(
            candidate_id=candidate_id, strategy_id=strategy_id, decision=decision, limit=limit
        )

    @app.get("/engine/diversity-events")
    async def engine_diversity_events(
        strategy_id: str | None = None,
        decision: str | None = None,
        limit: int = 100,
        authorization: str | None = Header(default=None),
    ...
    async def engine_portfolio_concentration_events(
        strategy_id: str | None = None,
        decision: str | None = None,
        limit: int = 100,
        authorization: str | None = Header(default=None),
        ...
        _auth(authorization)
        return await _repo().list_portfolio_concentration_events(
            strategy_id=strategy_id, decision=decision, limit=limit
        )

    @app.get("/engine/replay-result-links")
    async def engine_replay_result_links(
    ...
    async def engine_bandit_recommendations(
        strategy_id: str | None = None,
    ...
    async def engine_bandit_recommendations_run(
        request: EngineBanditRecommendationRunRequest, authorization: str | None = Header(default=None)
    ) -> dict[str, Any]:
        _auth(authorization)
        command = await _enqueue_command(
            _repo(), target_role="trader", command_type="engine_bandit_run", payload=request.model_dump(mode="json")
        )
        return _accepted_command(command)

    @app.get("/engine/evidence-packs/{evidence_pack_id}")
    async def engine_evidence_pack(
        evidence_pack_id: str, authorization: str | None = Header(default=None)
    ) -> dict[str, Any]:
        _auth(authorization)
    ...
    async def engine_debate_decisions(
        candidate_id: str | None = None, limit: int = 100, authorization: str | None = Header(default=None)
    ) -> list[dict[str, Any]]:
        _auth(authorization)
    ...
    async def engine_order_intents(
        execution_mode: str | None = None, limit: int = 100, authorization: str | None = Header(default=None)
    ) -> list[dict[str, Any]]:
        _auth(authorization)
        return await _repo().list_order_intents(execution_mode=execution_mode, limit=limit)

    @app.get("/engine/strategy-version-policies")
    async def engine_strategy_version_policies(
        state: str | None = None,
        strategy_id: str | None = None,
        limit: int = 500,
        authorization: str | None = Header(default=None),
        ...
        _auth(authorization)
        rows = await _repo().list_strategy_version_policies(
            state=state,
            strategy_id=strategy_id,
        ...
        return {
            ...
            "missing_exact_version_default": "research_only",
            "paper_requires_state": "paper_approved",
            "policies": rows,
        }

    @app.get("/engine/execution-cost-quotes")
    async def engine_execution_cost_quotes(
        candidate_id: str | None = None,
        ...
        _auth(authorization)
        return await _repo().list_execution_cost_quotes(
            candidate_id=candidate_id,
    ...
    async def engine_execution_reports(
        intent_id: str | None = None, limit: int = 100, authorization: str | None = Header(default=None)
    ) -> list[dict[str, Any]]:
        _auth(authorization)
    ...
    async def engine_reconciliation(
        execution_mode: str | None = None, limit: int = 100, authorization: str | None = Header(default=None)
    ) -> list[dict[str, Any]]:
        _auth(authorization)
    ...
    async def engine_model_versions(
        status: str | None = None,
    ...
    async def engine_pnl_attribution(
        strategy_id: str | None = None,
        asset: str | None = None,
        limit: int = 100,
        authorization: str | None = Header(default=None),
    ...
    async def engine_validation_report(
        limit: int = 500, authorization: str | None = Header(default=None)
    ) -> dict[str, Any]:
        _auth(authorization)
    ...
    @app.get("/engine/strategy-research")
    async def engine_strategy_research(
        window_hours: int = 24 * 30,
        as_of_ms: int | None = None,
        authorization: str | None = Header(default=None),
    ) -> dict[str, Any]:
        _auth(authorization)
        return await build_strategy_research_report(
            _repo(),
            window_hours=max(24, min(24 * 180, window_hours)),
            as_of_ms=as_of_ms,
            bootstrap_iterations=int(getattr(settings, "engine_promotion_bootstrap_iterations", 10_000)),
            min_effective_blocks=int(getattr(settings, "engine_promotion_min_effective_blocks", 30)),
        )
    ...
    async def engine_readiness(
        window_hours: int | None = None, limit: int = 1000, authorization: str | None = Header(default=None)
    ) -> dict[str, Any]:
        _auth(authorization)
        service = getattr(app.state, "engine_service", None)
    ...
    async def engine_paper_signoff_preflight(
        ...
        return await build_paper_signoff_preflight(
            _repo(), settings, service, symbols=symbol_list, window_hours=window_hours, limit=limit
        )

    @app.get("/engine/replay-comparisons")
    async def engine_replay_comparisons(
        limit: int = 100, authorization: str | None = Header(default=None)
    ) -> list[dict[str, Any]]:
        _auth(authorization)
    ...
    async def engine_replay_comparison_run(
        request: EngineReplayComparisonRequest, authorization: str | None = Header(default=None)
    ) -> dict[str, Any]:
        _auth(authorization)
        command = await _enqueue_command(
            _repo(),
            target_role="trader",
            command_type="engine_replay_comparison_run",
            payload=request.model_dump(mode="json"),
        )
        return _accepted_command(command)
