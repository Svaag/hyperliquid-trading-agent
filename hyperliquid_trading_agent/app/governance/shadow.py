...
class ShadowComparisonService:
    ...
    async def replay_candidate_diff(self, proposal_id: str, *, decision_id: str | None = None) -> ReplayResult:
        ...
        event_evaluations, evidence_summary = await self._load_replay_evidence(diff)
        transform = {"applied": False, "reason": "no_deterministic_event_transform"}
        baseline_metrics = _event_metrics(event_evaluations)
        ...
        if not transform["applied"]:
            status = "audit_only" if event_evaluations else "insufficient_data"
        result = ReplayResult(
            replay_id=f"replay_{uuid4().hex}",
            proposal_id=proposal_id,
            decision_id=decision_id,
    ...
    async def _load_replay_evidence(self, diff: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
        ...
        return (
            list(event_evaluations.values()),
            {
                ...
                "scope_fallback_used": bool(event_evaluations and not evidence_ids),
            },
        )
