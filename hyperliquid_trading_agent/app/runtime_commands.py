...
COMMAND_SPECS: tuple[WorkerCommandSpec, ...] = (
    ...
    WorkerCommandSpec(
        "newswire_reclassify",
        ServiceRole.NEWSWIRE,
        "_handle_reclassify",
        ("POST /newswire/reclassify",),
        "Reassess bounded persisted Newswire stories without publishing them to live consumers.",
        external_side_effect="db_write",
        handler_status="implemented",
    ),
    WorkerCommandSpec(
    ...
    WorkerCommandSpec(
        ...
        ServiceRole.TRADER,
        "_handle_engine_newswire_replay",
        ("POST /newswire/replay",),
        "Replay bounded Newswire history into timestamp-preserving engine evidence without moving the live offset.",
        external_side_effect="db_write",
        handler_status="implemented",
    ),
    WorkerCommandSpec(
        "engine_operator_proposal_ack",
        ServiceRole.TRADER,
        "_handle_engine_operator_proposal_ack",
        (
            "POST /engine/operator-proposals/{proposal_id}/acknowledge",
        ),
        "Acknowledge an institutional-engine proposal without creating a paper order.",
        external_side_effect="db_write",
        idempotency_key_fields=("proposal_id",),
        handler_status="implemented",
    ),
    WorkerCommandSpec(
        "engine_operator_proposal_reject",
        ServiceRole.TRADER,
        "_handle_engine_operator_proposal_reject",
        (
            "POST /engine/operator-proposals/{proposal_id}/reject",
        ),
        "Reject an institutional-engine operator proposal.",
        external_side_effect="db_write",
        idempotency_key_fields=("proposal_id",),
        handler_status="implemented",
    ),
    WorkerCommandSpec(
        "engine_validation_monitor_run_once",
        ServiceRole.TRADER,
        "_handle_engine_validation_monitor_run_once",
        ("POST /engine/validation-monitor/run-once",),
        "Run one trader-owned validation digest and enqueue its operator notification.",
        external_side_effect="db_write",
        handler_status="implemented",
    ),
    WorkerCommandSpec(
